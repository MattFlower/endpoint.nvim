...
function JakartaEEParser:extract_base_path(file_path, line_number)
  ...
  local method_node = self:_find_method_for_line(tree, source, line_number)
...
function JakartaEEParser:extract_method(content)
  ...
  -- Check for HTTP method annotations using the lookup table
  for method in pairs(HTTP_METHODS) do
    if normalized_content:match("@" .. method) then
      return method
    end
  end
...
function JakartaEEParser:parse_content(content, file_path, line_number, column)
  ...
  return self:_parse_with_treesitter(tree, source, file_path, line_number, column)
end

---Gets parsing confidence for JAX-RS annotations
---@param content string The annotation content
---@return number confidence between 0.0 and 1.0
...
---@return string|nil The normalized path, or nil if input was nil
function JakartaEEParser:_normalize_path(path)
  if not path then
    return nil
  end
...
---@return table|nil endpoint data
function JakartaEEParser:_parse_with_treesitter(tree, source, file_path, line_number, column)
...
---Finds a method declaration for the given line (tries direct lookup, then nearby)
---@param tree table TreeSitter tree
---@param source string Source code
---@param line_number number 1-indexed line number
---@return table|nil method_node
function JakartaEEParser:_find_method_for_line(tree, source, line_number)
  local root = tree:root()
  local ts_line = line_number - 1 -- TreeSitter uses 0-indexed

  -- First try: find node at line and walk up to method_declaration
  ...
  -- Second try: query all methods and find one containing this line with JAX-RS annotations
  ...
  for _, method_node in query:iter_captures(root, source) do
...
---Extracts string literals from a binary expression (concatenation)
